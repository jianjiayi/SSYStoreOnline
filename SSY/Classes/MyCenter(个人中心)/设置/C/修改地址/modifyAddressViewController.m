...
@interface modifyAddressViewController ()<UITableViewDelegate,UITableViewDataSource,NSURLSessionDelegate,UIGestureRecognizerDelegate>
...
-(void)setNavBar{
    ...
    delBtn.titleLabel.font = PFR16Font;
    [delBtn setTitleColor:RGB(102, 102, 102) forState:UIControlStateNormal];
    [delBtn addTarget:self action:@selector(deleBtnClick) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithCustomView:delBtn];
...
-(void)createData{
    ...
    address.textVal = @"美丽园小区  15号楼  2单元801室";
    address.key = @"address";
...
- (void)textFieldValueChange:(NSNotification *)note
{
    ...
    if(name.length != 0 && phone.length != 0 && dist.length != 0 ){
